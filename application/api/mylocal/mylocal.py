...
def _warmup():
        ...
        # List of table-controller arguments
        controllers = [
        ...
        # Row ID to fetch from each table
        row_id = 'LK-1127015'
        def fetch_row(table_name):
            """Fetch a row from the given table."""
            controller = TableController(table_name)
            return controller.get_table_row(row_id)


        # Execute all fetch operations in parallel
        with concurrent.futures.ThreadPoolExecutor() as executor:
            # Submit all tasks to the executor
            futures = {executor.submit(fetch_row, table): table for table in controllers}

        # Collect and display results as tasks complete
